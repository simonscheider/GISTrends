...
pacman::p_load(readr,rvest,urltools,uuid,RSQLite,rjson,rgdal,curl,gtrendsR,jsonlite,classInt,grid,
               dplyr,tmap,rmapshaper,ggplot2,reshape2,foreach,gtools,tmaptools,lattice)
...
bivariate.choropleth <- function(fnout, bivmap.dataset, bivmap.vars, bivmap.labels, bivmap.style='quantile'){
  pdf(fnout)
...
ams_gm_sdf_simpl = rmapshaper::ms_simplify(ams_gm_sdf, keep = .01, keep_shapes = T )
...
FUN_subgroups(gt_df_quart_sum_diff2017, c('low_search_volume','GTRENDS_MODE','geo'), function(block){
  ...
  fn = paste0(OUT_DIR,'gm_trend_map-',row$low_search_volume,'-',row$GTRENDS_MODE,'-',row$geo,'-diff_2007_2017.pdf')
  print(fn)
  print(nrow(block))
  ...
  gtrends_map <- tm_shape(gm_gtrends_sdf) +
      tm_fill(col="hits_sum_diff_pc", style="equal", palette="PRGn", n = 5) + #quantile , 
...
FUN_subgroups(gtgmyear_df, c('year'), function(block){
  ...
  gtrends_map <- tm_shape(gm_gtrends_sdf) +
      ...
      #tm_scale_bar(width=0.15,position=c("left","bottom")) +
      #tm_credits("\n\n\n\n\nContains data from Office for National Statistics\nand Ordnance Survey. Crown copyright & database right 2016.",size=0.6,position=c("left","bottom")) + 
      tm_layout(frame=FALSE,title = tit,legend.position = c('right','bottom'), title.size = 1) + tm_legend(scale=0.75) + tm_scale_bar(position=c("left", "bottom"),breaks = SCALE_BREAKS) +
      #tm_text(text = "ISO_A2",size = 0.1) +
...
FUN_subgroups(gtgmyear_diff_df, c('year'), function(block){
  ...
  gtrends_map <- tm_shape(gm_gtrends_sdf) +
      ...
      tm_legend(scale=0.75) + tm_scale_bar(position=c("left", "bottom"),breaks = SCALE_BREAKS) +
      #tm_text(text = "ISO_A2",size = 0.1) +
      tm_borders(col = 'white',lty = 1.3)
...
names(sdf)
nrow(sdf)
fn = paste0(OUT_DIR,'gm_trend_map-between_gm-2017-pop_vs_gtrends.pdf')
bivariate.choropleth(fn, sdf,c('gt_sum_2017','tot_pop'),c('GTrends idx 2017','Population'))

# gtrends interest per 100k people (2017)
sdf$gt_per100kppl = sdf$gt_sum_2017 / (sdf$tot_pop/100000)

m <- tm_shape(sdf) +
    tm_fill(col="gt_per100kppl", style='quantile', palette="Blues", n = 5) + #quantile , PRGn
    #tm_credits("\n\n\n\n\nContains data from Office for National Statistics\nand Ordnance Survey. Crown copyright & database right 2016.",size=0.6,position=c("left","bottom")) + 
    tm_layout(frame=FALSE,title = 'Google Trends interest per 100k residents (2017)',legend.position = c('right','bottom'), title.size = 1) +
    tm_legend(scale=0.75) + tm_scale_bar(position=c("left", "bottom"),breaks = SCALE_BREAKS) +
    #tm_text(text = "ISO_A2",size = 0.1) +
    tm_borders(col = 'white',lty = 1.3)
    
save_tmap(m,paste0(OUT_DIR,'gm_trend_map-between_gm-2017-gtrends_100k_people.pdf'))
rm(m)
...
gen_hotel_linechart = function( df ){
  ...
  #df$VAL = as.factor(as.character(df$VAL))
  #View(df)
  p<-ggplot(data=df, aes_string(x='year', y='visitor_n', group='source_country_iso2', 
