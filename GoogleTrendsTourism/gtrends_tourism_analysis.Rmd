...
add_nl_basemap = function(){
  sdf = gBuffer(nl_bounds_sdf_clipped, byid = T, width = -400)
  tm_shape(sdf) +
  #tm_borders(col = 'gray80', lty = 'dashed', alpha = 1)
  tm_borders(col = 'gray93', lty = 'solid', alpha = 1, lwd = 2.5)
}

my_scale_bar = function(){
  tm_scale_bar(width=0.15,position=c("left","bottom"),breaks = SCALE_BREAKS, size=1.04)
}
```
...
FUN_subgroups( gt_countries, c('year','keyword','low_search_volume'), function(df){
  y = unique(df$year)
  ...
  gtrends_map <- tm_shape(mdf) +
    tm_fill(col="hits_asinh", style="equal", palette="Oranges", 
